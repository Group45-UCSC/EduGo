...
const childList = async (req, res) => {
    ...
    const childData = await pool.query(
      "SELECT * FROM children c INNER JOIN registered_users u ON u.user_id = c.parent_id INNER JOIN school_ride r ON r.ride_id = c.ride_id INNER JOIN vehicle v ON v.vehicle_id = r.vehicle_id",
    );
