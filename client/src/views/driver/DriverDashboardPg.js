  ...
  const fetchImage = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/edugo/driver/vehicle/viewImg/${userId}`
      );
      const imgUrl = await response.json();
      setImageDataUrl(imgUrl);
      // console.log(imgUrl);
      // setImageSrc(`/uploads/${data.filename}`);
      // Update the path according to your actual file structure
    } catch (error) {
                    ...
                    <div className="flex gap-x-20">
                      <div className="w-40 ">
                        {/* <img
                            ...
                            ></img> */}
                        {/* {imageSrc && <img src={imageSrc} alt="Uploaded Vehicle" />} */}
                        {imageDataUrl && (
                          <img src={imageDataUrl} alt="Uploaded Vehicle" />
                        )}
                      </div>
                      <div className="">
